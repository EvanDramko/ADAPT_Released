...
def normalize_batch_padded(X_pad, mask, x_mean, x_std):
    """
    Normalize X only where mask==True.
    Pads remain 0.
    """
    device = X_pad.device
    ...
    feat = X_pad
    mean = x_mean.view(1,1,-1)
    std  = x_std.view(1,1,-1)

    # Only touch valid tokens
    Xn_feat = (feat - mean) / (RESCALE_FACTOR * (std + eps))
    Xn = torch.where(m, Xn_feat, torch.zeros_like(Xn_feat))
    return Xn
...
def fit_stats_from_ragged(X_list: List[torch.Tensor], Y_list: List[torch.Tensor]) -> Dict[str, torch.Tensor]:
    ...
    # Treat xyz coordinates as one entity for stats.
    if X_stack.shape[1] >= 3:
        xyz_mean = X_stack[:, :3].mean()
        xyz_std = X_stack[:, :3].std(unbiased=False).clamp_min(1e-8)
        x_mean[:3] = xyz_mean
        x_std[:3] = xyz_std

    # Treat fx/fy/fz as one entity for stats.
    if Y_stack.shape[1] >= 3:
        force_mean = Y_stack[:, :3].mean()
        force_std = Y_stack[:, :3].std(unbiased=False).clamp_min(1e-8)
        y_mean[:3] = force_mean
        y_std[:3] = force_std
...
def normalize_ragged_with_stats(
    ...
    if int(x_mean.shape[-1]) != int(X_stack.shape[-1]) or int(x_std.shape[-1]) != int(X_stack.shape[-1]):
        raise ValueError(
            f"X stats dimension mismatch: X has {X_stack.shape[-1]} features, "
            f"x_mean has {x_mean.shape[-1]}, x_std has {x_std.shape[-1]}"
        )
    if int(y_mean.shape[-1]) != int(Y_stack.shape[-1]) or int(y_std.shape[-1]) != int(Y_stack.shape[-1]):
        raise ValueError(
            f"Y stats dimension mismatch: Y has {Y_stack.shape[-1]} features, "
            f"y_mean has {y_mean.shape[-1]}, y_std has {y_std.shape[-1]}"
        )

    Xn_stack = normalize_all(0, X_stack, mean=x_mean, std=x_std)
