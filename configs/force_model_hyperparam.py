...
@dataclass(frozen=True)
class DataConfig:
    train_path: str = "./data/raggedTrain_weighted.pt"
    test_path: str = "./data/raggedTest_weighted.pt"
    has_weighting: bool = True
...
@dataclass(frozen=True)
class ModelPaths:
    pretrainPath: Optional[str] = "./saved_models/retrained_baseline.pth" # load is saved pretrained model
...
class TrainConfig:
    epochs: int = 80
    alt_loss_fxn: Optional[Callable[[float, float], float]] = None
